...
void main() {
        ...
        // regular noise
        // vec2 displacedUv = textureCoord + cnoise(vec3(textureCoord * 1.0, u_time * 0.002 + u_mouse.y)) * u_noisyEffect;
        // vec2 strength = displacedUv + cnoise(vec3(displacedUv * 2.0, u_time * 0.002 + u_mouse.x )) * u_noisyEffect;


        // vec3 textureColor = texture2D(uSampler0, strength).rgb;
